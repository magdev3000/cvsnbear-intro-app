...
class CustomDropDown extends Component {
  ...
  togglePopover = () => {
    this.setState(({ active }) => {
      if (active && this.props.eventName) {
        amplitude.logEvent(this.props.eventName)
      }
  ...
  render() {
    const { options, input, classNameInputValue } = this.props;

    const indexValue = options && Array.isArray(options) ? findIndex(options, ['value', input.value]) : -1;

    return (
      <div className="CustomDropDown" ref={this._Ref} onClick={input.onClick}>
